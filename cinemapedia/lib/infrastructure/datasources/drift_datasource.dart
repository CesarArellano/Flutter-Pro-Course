...
class DriftDatasource implements LocalStorageDatasource {
  ...
  Future<bool> isMovieFavorite(int movieId) async {
    final favoriteMovie = await (db.select(db.moviesTable)
  ...
  Future<List<Movie>> loadMovies({int limit = 10, offset = 0}) async {
    final entries = await (db.select(db.moviesTable)
  ...
  Future<void> toggleFavorite(Movie movie) async {
    final favoriteMovie = await (db.select(db.moviesTable)
      ..where((entry) => entry.id.equals(movie.id!))
    ).getSingleOrNull();

    if (favoriteMovie != null) {
      await (db.delete(db.moviesTable)
        ..where((entry) => entry.id.equals(movie.id!))
      ).go();
      return;
    }
  ...
  MoviesTableCompanion _movieToCompanion(Movie movie) => MoviesTableCompanion.insert(
    id: Value(movie.id!),
